...
$P(w_1, w_2,w_3,w_4)=p(w_4|w_1,w_2,w_3) * p(w_1,w_2,w_3)=p(w_4|w_1,w_2,w_3) * p(w_3|w_1,w_2) * p(w_2|w_1) * p(w_1)$   
...
例如$$P(今天天气不错啊) = p(今天) * p(天气|今天) * p(不错|今天,天气) * p(啊|今天,天气,不错)$$
...
他提出引入了 **Skip-gram 和 CBOW** 两种高效训练算法，以及针对NNLM计算量消耗最大的最后一步提出了**层次softmax和negative sample负样本采样**这两种方法优化
而且**它的核心功能就是得到word embedding词向量**，用于下游任务。它是一个快速高效的词嵌入工具。人们也逐渐意识到词向量的用处。
...
Seq2Seq模型更像是一种结构：编码器解码器结构(Encoder-Decoder)。这总结构主要应对翻译这种把**一个序列转换为另一个序列**的任务。  
当时的Seq2Seq中，解码器编码器主要是使用RNN模型(LSTM，GRU)。RNN(循环神经网络)在结构上适合对序列建模，于是自然应用到了自然语言处理。  
...
Elmo的**主要功能就是提供词表征(词向量)**。因为在当时，词向量凭借对神经网络的无缝接入，使其应用十分广泛
...
而且BERT它对词向量的构建更加精准，毕竟左右都看一次和只向左看一次能得都更多信息。
...
也一步骤也可以认为是激发模型的**涌现能力**，向一个扳机/trigger。
**安全性**
